...
import { getRequestedLoanById, removeRequestedLoanById, requestStatusEnum } from "./loanRequestsSlice";
	...
	const handleRequestResponse = (response: string): void => {
		switch (response) {
			case "accept":
				...
				navigate(`/accepted-loan-requests/${loanRequestId}`);
				break;
			case "deny":
				if (loanRequestId) {
				...
				navigate(`/denied-loan-requests/${loanRequestId}`);
				break;
			case "counter offer":
	...
	return (
		<div className="w-full h-full bg-slate-900 p-20 flex items-center justify-center">
			<div className="w-full h-full flex flex-col justify-center items-center gap-5 p-10 bg-slate-800">
				<h1 className="text-yellow-500 text-center text-xl">Request from: <Link to="/friends/:friendId" className="underline font-bold text-white">{selectedLoanRequest?.fromUserId}</Link></h1>
				...
				{selectedLoanRequest?.requestStatus === requestStatusEnum.PENDING && buttons}
			</div>
		</div>
	);
}
